...
The takeway here is that when we talk about regulations, we have to be specific since their goals, mechanisms and therefore impacts are going to be different. Unfortunately, commentators criticizing rent controls are quick to conflate all rent regulation with the kind experienced in New York City, which Lind identifies as an extreme version of a type E control.[^lind-2001] 
...
But that's a false choice. New York City is a very idiosyncratic example &mdash; and so we're not limited to choosing between an unfettered market and a ruinously restrictive price control. 

Consider that in a perfectly competitive market, prices should approach the marginal cost and, capital expenditures aside, in principle therefore it's not really reasonable to expect rents to grow faster than inflation.  Of course, housing markets as we experience them are far from perfectly competitive; for starters, supply is inelastic over the short to medium term. Arnott goes on to argue that housing units are highly heterogenous (i.e. not fungible commodities) and that since markets suffer from a variety of externalities and high transaction costs, and so on, such that a well-designed control could actually _improve_ on markets' unrestricted equilibrium.[^arnott-1995] [^nota-bene-2]

<h3>Ontario's rent control is rather soft</h3>

Recall that Ontario's current rent control regime, which Lind categorized as a type B control,[^lind-2001] pegs rents to inflation, does not control rents between tenants, and allows cost pass throughs.[^depressing]
...
[^nota-bene-2]: I've glossed over a much more animated academic discussion than what I have presented here. For a more elaborate and exhaustive treatment of the literature, see Lind (2001, 2003),[^lind-2001] [^lind-2003], Brescia (2005)[^brescia-2005], Grant (2011),[^grant-2011] and Ambrosius et al (2015).[^ambrosius-2015]

[^depressing]: The depressing thing about writing so much on this topic is precisely that Ontario's current rent control regime is _so tame_.

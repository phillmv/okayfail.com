...
However, we're not limited to choosing between an unfettered market and a ruinously restrictive price control. After all, our purpose is to protect tenants from runaway price inflation. Given the long-term nature of multi-residential investment, their business model can be satisfied so long as cashflows keep up with costs. 
...
After all, in a laissez faire but well-functioning market, there is no guarantee that rents will rise above inflation. A cost-adjusted tenancy control, then, primarily impacts only one area of the development process: the initial lease up of an empty building.[^tait-2017] As long as developers have the ability to adjust for initial mistakes, over the long run the impact on their finances should be minimal.
...
<h3>Reminder: economic rents are unproductive</h3>
